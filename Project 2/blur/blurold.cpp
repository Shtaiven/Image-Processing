...
void blur(imageP I1, int xsz, int ysz, imageP I2)
{
	...
	for(y = 0; y < total; y += I1->width) {
		...
		for(x = 0; x < I1->width; ++x) {
			out[x+y] = sum/xsz;
			sum += (hbuf[x+xsz]-hbuf[x]); // this line is likely the problem
		}
	}
	...
	for(x = 0; x < I1->width; ++x) {
		...
		for (i = 0; i < I1->height; ++i) {
			temp_in[i] = in[x+(i*I1->width)]; // x is the column, i*I1->width is how far the next pixel in the column is from the first
		}
		...
		for(y = j = 0; y < total; y += I1->width) {
			out[x+y] = sum/ysz;
			sum += (vbuf[++j+ysz]-vbuf[j]); // this likely the problem
		}
	}
